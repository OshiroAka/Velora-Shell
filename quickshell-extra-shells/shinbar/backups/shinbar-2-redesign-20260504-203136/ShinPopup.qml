...
QtObject {
    ...
    // Eventos de teclado enviados para o popup aberto.
    property int insideDelta: 0
    property int insideX: 0
    property int insideY: 0
    property int insideNonce: 0
    property int activateNonce: 0
    ...
    function navigateInside(dx, dy) {
        insideX = dx
        insideY = dy
        insideDelta = Math.abs(dx) > 0 ? dx : dy
        insideNonce += 1
    }

    function activateInside() {
        activateNonce += 1
    }
    ...
    function openFocused() {
        ...
        if (focusTarget === "media") {
            active = active === "media-tab" ? "" : "media-tab"
            return
        }
