...
Item {
    ...
    property real introSidebar: 1.0
    property real introContent: 1.0
    property real introFooter: 1.0
    property real sweepX: -180
    property var wallpapers: []
    property var fonts: []
    property int keyboardRow: 0
    property var categories: ["bar", "wallpapers", "clock", "lock", "weather", "profile", "media", "search", "fonts", "colors", "effects"]
    ...
    function rowCount() {
        ...
        if (ShinData.settingsCategory === "wallpapers") return 11
        if (ShinData.settingsCategory === "clock") return 7
        if (ShinData.settingsCategory === "lock") return 6
    ...
    function setCat(c) {
        ...
        keyboardRow = 0
        Qt.callLater(root.ensureCategoryData)
    }
    ...
    function adjustKeyboard(dir) {
        ...
        if (cat === "bar") {
        ...
        } else if (cat === "wallpapers") {
            if (r === 0) setBoolKey("wallpaperSelectorEnabled", ShinConfig.wallpaperSelectorEnabled, dir)
            else if (r === 1) cycleStep("wallpaperModel", ShinConfig.wallpaperModel, 5, step)
            else if (r === 2) cycleStep("wallpaperPanelPosition", ShinConfig.wallpaperPanelPosition, 4, step)
            else if (r === 3) cycleStep("wallpaperOpenStyle", ShinConfig.wallpaperOpenStyle, 4, step)
            else if (r === 4) cycleStep("wallpaperMoveStyle", ShinConfig.wallpaperMoveStyle, 5, step)
            else if (r === 5) cycleStep("wallpaperTransition", ShinConfig.wallpaperTransition, 4, step)
            else if (r === 6) setInt("wallpaperPanelThick", ShinConfig.wallpaperPanelThick + step * 10, 140, 420)
            else if (r === 7) setReal("wallpaperPanelSpan", ShinConfig.wallpaperPanelSpan + step * 0.05, 0.45, 1)
            else if (r === 8) setBoolKey("wallpaperFillEdges", ShinConfig.wallpaperFillEdges, dir)
            else if (r === 9) setBoolKey("wallpaperApplyPywal", ShinConfig.wallpaperApplyPywal, dir)
            else if (r === 10 && dir >= 0) ShinPopup.open("wallpapers")
        } else if (cat === "clock") {
            ...
            if (r === 0) setBoolKey("lockEnabled", ShinConfig.lockEnabled, dir)
            else if (r === 1) setReal("lockBlur", ShinConfig.lockBlur + step * 0.05, 0.38, 1)
            else if (r === 2) setReal("lockDim", ShinConfig.lockDim + step * 0.05, 0.1, 0.42)
            else if (r === 3) setReal("lockGlow", ShinConfig.lockGlow + step * 0.05, 0, 1.6)
            else if (r === 4) cycleStep("lockAnimStyle", ShinConfig.lockAnimStyle, 3, step)
            else if (r === 5) setReal("lockAnimStrength", ShinConfig.lockAnimStrength + step * 0.05, 0.35, 1.8)
        } else if (cat === "weather") {
    ...
    function wallpaperModelName(v) {
        if (v === 0) return "Pill"
        if (v === 1) return "Cards"
        if (v === 3) return "Spotlight"
        if (v === 4) return "Varal"
        return "Skew"
    }

    function wallpaperPositionName(v) {
        if (v === 1) return "Topo"
        if (v === 2) return "Esquerda"
        if (v === 3) return "Direita"
        return "Baixo"
    }

    function wallpaperOpenName(v) {
        if (v === 1) return "Rise"
        if (v === 2) return "Fade"
        if (v === 3) return "Zoom"
        return "Slide"
    ...
    function wallpaperMoveName(v) {
        if (v === 1) return "Shooter"
        if (v === 2) return "Trail"
        if (v === 3) return "Snap"
        if (v === 4) return "Fade"
        return "Elastic"
    ...
    function wallpaperTransitionName(v) {
        if (v === 1) return "Grow"
        if (v === 2) return "Wipe"
        if (v === 3) return "Outer"
        return "Fade"
    ...
    function lockLineName(v) {
        if (v === 1) return "Diamantes"
        if (v === 2) return "Sem linhas"
        return "Pilares"
    }

    function lockAnimName(v) {
        if (v === 1) return "Pilares"
        if (v === 2) return "Vidro"
        return "Cascata"
    }

    function replaySettingsEntrance() {
        introSidebarAnim.restart()
        introContentAnim.restart()
        introFooterAnim.restart()
        sweepAnim.restart()
    }

    function categoryUsesWallpapers(cat) {
        return cat === "clock" || cat === "lock" || cat === "weather" || cat === "profile" || cat === "media"
    }

    function ensureCategoryData() {
        if (!opened)
            return
        if (categoryUsesWallpapers(ShinData.settingsCategory) && wallpapers.length === 0 && !wallProc.running)
            wallProc.running = true
        if (ShinData.settingsCategory === "fonts" && fonts.length === 0 && !fontProc.running)
    ...
    NumberAnimation {
        ...
        from: -220
        to: 920
        duration: ShinData.popupAnim(860)
        easing.type: Easing.OutCubic
    }
    ...
    SequentialAnimation {
        ...
        ScriptAction { script: root.introSidebar = 0.0 }
    ...
    SequentialAnimation {
        ...
        ScriptAction { script: root.introContent = 0.0 }
        PauseAnimation { duration: ShinData.popupAnim(150) }
        NumberAnimation {
            target: root
    ...
    SequentialAnimation {
        ...
        PauseAnimation { duration: ShinData.popupAnim(250) }
        NumberAnimation {
            target: root
    ...
    Process {
        ...
        running: false
        property var tmp: []
        command: ["/home/shira/.config/quickshell/shinbar/scripts/shinbar-list-wallpapers"]
    ...
    Process {
        ...
        running: false
        property var tmp: []
        command: ["/home/shira/.config/quickshell/shinbar/scripts/shinbar-fonts"]
    ...
    PanelWindow {
        ...
        Repeater {
            ...
            Rectangle {
                ...
                z: 1
                x: card.x + Math.round(root.lerp(18, 0, et))
                y: card.y + Math.round(root.lerp(-10, 0, et))
                width: card.width
                height: card.height
                radius: card.radius
                antialiasing: true
                ...
                color: Qt.rgba(ShinColors.accent.r, ShinColors.accent.g, ShinColors.accent.b, 0.11)
                border.width: 1
                border.color: Qt.rgba(ShinColors.accent.r, ShinColors.accent.g, ShinColors.accent.b, 0.21)
            }
        }
        ...
        Rectangle {
            ...
            height: Math.min(760, overlay.height - ShinConfig.barH - ShinConfig.barMarginT - 28)
            ...
            Rectangle {
                ...
                y: -45
                rotation: 13
                color: Qt.rgba(ShinColors.accent.r, ShinColors.accent.g, ShinColors.accent.b, 0.052)
            }
            ...
            Row {
                ...
                Column {
                    ...
                    transform: Translate {
                        x: -20 * (1.0 - root.introSidebar)
                        y: 8 * (1.0 - root.introSidebar)
                    }
                    ...
                    CatButton { label: "Wallpapers"; cat: "wallpapers" }
                    CatButton { label: "Clock"; cat: "clock" }
                    CatButton { label: "Lock"; cat: "lock" }
                ...
                Item {
                    ...
                    transform: Translate {
                        x: 22 * (1.0 - root.introContent)
                        y: 10 * (1.0 - root.introContent)
                    }

                    Column {
                        anchors.fill: parent
                        spacing: 12
                        visible: ShinData.settingsCategory === "wallpapers"
                        ...
                        ToggleRow { keyIndex: 0; title: "Ativar seletor"; active: ShinConfig.wallpaperSelectorEnabled; onToggle: root.setBool("wallpaperSelectorEnabled", !ShinConfig.wallpaperSelectorEnabled) }
                        CycleRow { keyIndex: 1; title: "Modelo visual"; valueText: root.wallpaperModelName(ShinConfig.wallpaperModel); onNext: root.cycleInt("wallpaperModel", ShinConfig.wallpaperModel, 5) }
                        CycleRow { keyIndex: 2; title: "Posição do painel"; valueText: root.wallpaperPositionName(ShinConfig.wallpaperPanelPosition); onNext: root.cycleInt("wallpaperPanelPosition", ShinConfig.wallpaperPanelPosition, 4) }
                        CycleRow { keyIndex: 3; title: "Entrada"; valueText: root.wallpaperOpenName(ShinConfig.wallpaperOpenStyle); onNext: root.cycleInt("wallpaperOpenStyle", ShinConfig.wallpaperOpenStyle, 4) }
                        CycleRow { keyIndex: 4; title: "Movimento"; valueText: root.wallpaperMoveName(ShinConfig.wallpaperMoveStyle); onNext: root.cycleInt("wallpaperMoveStyle", ShinConfig.wallpaperMoveStyle, 5) }
                        CycleRow { keyIndex: 5; title: "Transição"; valueText: root.wallpaperTransitionName(ShinConfig.wallpaperTransition); onNext: root.cycleInt("wallpaperTransition", ShinConfig.wallpaperTransition, 4) }
                        SettingRow { keyIndex: 6; title: "Espessura"; valueText: ShinConfig.wallpaperPanelThick + "px"; onMinus: root.setInt("wallpaperPanelThick", ShinConfig.wallpaperPanelThick - 10, 140, 420); onPlus: root.setInt("wallpaperPanelThick", ShinConfig.wallpaperPanelThick + 10, 140, 420) }
                        SettingRow { keyIndex: 7; title: "Span"; valueText: Math.round(ShinConfig.wallpaperPanelSpan * 100) + "%"; onMinus: root.setReal("wallpaperPanelSpan", ShinConfig.wallpaperPanelSpan - 0.05, 0.45, 1); onPlus: root.setReal("wallpaperPanelSpan", ShinConfig.wallpaperPanelSpan + 0.05, 0.45, 1) }
                        ToggleRow { keyIndex: 8; title: "Preencher bordas"; active: ShinConfig.wallpaperFillEdges; onToggle: root.setBool("wallpaperFillEdges", !ShinConfig.wallpaperFillEdges) }
                        ToggleRow { keyIndex: 9; title: "Aplicar pywal"; active: ShinConfig.wallpaperApplyPywal; onToggle: root.setBool("wallpaperApplyPywal", !ShinConfig.wallpaperApplyPywal) }
                        ActionRow { keyIndex: 10; title: "Abrir seletor"; valueText: "shinbar"; onAction: ShinPopup.open("wallpapers") }
                    }
                    ...
                    Column {
                        ...
                        SettingRow { keyIndex: 5; title: "Opacidade da imagem do card"; valueText: Math.round(ShinData.clockBgOpacity * 100) + "%"; onMinus: root.setReal("clockBgOpacity", ShinData.clockBgOpacity - 0.05, 0, 1); onPlus: root.setReal("clockBgOpacity", ShinData.clockBgOpacity + 0.05, 0, 1) }
                        ...
                        WallpaperPicker {
                            title: "Imagem do card do clock"
                            targetKey: "clockBg"
                            currentPath: ShinData.clockBg
                            wallpapers: root.wallpapers
                            activePicker: ShinData.settingsCategory === "clock"
                        }
                    }
                    ...
                    Column {
                        anchors.fill: parent
                        spacing: 12
                        visible: ShinData.settingsCategory === "lock"
                        ...
                        ToggleRow { keyIndex: 0; title: "Ativar lockscreen"; active: ShinConfig.lockEnabled; onToggle: root.setBool("lockEnabled", !ShinConfig.lockEnabled) }
                        SettingRow { keyIndex: 1; title: "Blur do fundo"; valueText: Math.round(ShinConfig.lockBlur * 100) + "%"; onMinus: root.setReal("lockBlur", ShinConfig.lockBlur - 0.05, 0.38, 1); onPlus: root.setReal("lockBlur", ShinConfig.lockBlur + 0.05, 0.38, 1) }
                        SettingRow { keyIndex: 2; title: "Escurecimento"; valueText: Math.round(ShinConfig.lockDim * 100) + "%"; onMinus: root.setReal("lockDim", ShinConfig.lockDim - 0.05, 0.1, 0.42); onPlus: root.setReal("lockDim", ShinConfig.lockDim + 0.05, 0.1, 0.42) }
                        SettingRow { keyIndex: 3; title: "Glow das linhas"; valueText: Math.round(ShinConfig.lockGlow * 100) + "%"; onMinus: root.setReal("lockGlow", ShinConfig.lockGlow - 0.05, 0, 1.6); onPlus: root.setReal("lockGlow", ShinConfig.lockGlow + 0.05, 0, 1.6) }
                        CycleRow { keyIndex: 4; title: "Animação"; valueText: root.lockAnimName(ShinConfig.lockAnimStyle); onNext: root.cycleInt("lockAnimStyle", ShinConfig.lockAnimStyle, 3) }
                        SettingRow { keyIndex: 5; title: "Intensidade da animação"; valueText: Math.round(ShinConfig.lockAnimStrength * 100) + "%"; onMinus: root.setReal("lockAnimStrength", ShinConfig.lockAnimStrength - 0.05, 0.35, 1.8); onPlus: root.setReal("lockAnimStrength", ShinConfig.lockAnimStrength + 0.05, 0.35, 1.8) }
                        ...
                        Row {
                            ...
                            WallpaperPicker {
                                title: "Fundo da lockscreen"
                                targetKey: "lockWallpaper"
                                currentPath: ShinData.lockWallpaper
                                wallpapers: root.wallpapers
                                activePicker: ShinData.settingsCategory === "lock"
                                pickerWidth: 500
                                pickerHeight: 236
                            }
                        }
                    ...
                    Column {
                        ...
                        SettingRow { keyIndex: 0; title: "Opacidade da imagem do card"; valueText: Math.round(ShinData.weatherBgOpacity * 100) + "%"; onMinus: root.setReal("weatherBgOpacity", ShinData.weatherBgOpacity - 0.05, 0, 1); onPlus: root.setReal("weatherBgOpacity", ShinData.weatherBgOpacity + 0.05, 0, 1) }
                        ActionRow { keyIndex: 1; title: "Atualizar lista de fotos"; valueText: "~/Pictures/Wallpapers/static"; onAction: wallProc.running = true }

                        WallpaperPicker {
                            title: "Imagem do card da previsão"
                            targetKey: "weatherBg"
                            currentPath: ShinData.weatherBg
                            wallpapers: root.wallpapers
                            activePicker: ShinData.settingsCategory === "weather"
                        }
                    }
                    ...
                    Column {
                        ...
                        WallpaperPicker {
                            ...
                            wallpapers: root.wallpapers
                            activePicker: ShinData.settingsCategory === "profile"
                        }
                    }
                    ...
                    Column {
                        ...
                        WallpaperPicker {
                            ...
                            wallpapers: root.wallpapers
                            activePicker: ShinData.settingsCategory === "media"
                        }
                    }
            ...
            component CatButton: Rectangle {
                ...
                height: 36
                radius: 11
                scale: area.containsMouse || ShinData.settingsCategory === cb.cat ? 1.025 : 1.0
            ...
            component ToggleRow: Rectangle {
                ...
                Rectangle {
                    ...
                    Rectangle {
                        ...
                        Behavior on x { NumberAnimation { duration: ShinData.anim(120); easing.type: Easing.OutCubic } }
                        Behavior on color { ColorAnimation { duration: ShinData.anim(120); easing.type: Easing.OutCubic } }
                    }
            ...
            component WallpaperPicker: Column {
                ...
                property var wallpapers: []
                property bool activePicker: true
                property int pickerWidth: 500
                property int pickerHeight: 236

                width: wp.pickerWidth
                spacing: 8
                ...
                Rectangle {
                    ...
                    GridView {
                        anchors.fill: parent
                        anchors.margins: 8
                        model: wp.activePicker ? wp.wallpapers : []
                        cellWidth: Math.max(108, Math.floor(width / Math.max(1, Math.floor(width / 116))))
                        cellHeight: wp.pickerHeight <= 150 ? 74 : 92
                        cacheBuffer: 60
                        clip: true
                        ...
                        delegate: Rectangle {
                            ...
                            color: modelData === wp.currentPath
                                ...
                                : pickArea.containsMouse
                                    ...
                                    : Qt.rgba((ShinColors && ShinColors.fg ? ShinColors.fg : "#ffffff").r, (ShinColors && ShinColors.fg ? ShinColors.fg : "#ffffff").g, (ShinColors && ShinColors.fg ? ShinColors.fg : "#ffffff").b, 0.035)
                            border.width: modelData === wp.currentPath ? 2 : 1
                            border.color: modelData === wp.currentPath
                            ...
                            Rectangle {
                                ...
                                height: 24
                                color: Qt.rgba((ShinColors && ShinColors.bg ? ShinColors.bg : "#000000").r, (ShinColors && ShinColors.bg ? ShinColors.bg : "#000000").g, (ShinColors && ShinColors.bg ? ShinColors.bg : "#000000").b, 0.76)
                            }
                            ...
                            Behavior on color { ColorAnimation { duration: ShinData.anim(100); easing.type: Easing.OutCubic } }
                        }
                    }
