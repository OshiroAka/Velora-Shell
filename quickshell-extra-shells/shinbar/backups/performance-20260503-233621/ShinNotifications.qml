...
PanelWindow {
    ...
    onPanelOpenChanged: {
        ...
        if (panelOpen) {
            openNonce += 1
            selectedIndex = historyModel.count > 0 ? 0 : -1
            loadHistory()
        } else {
            selectedIndex = -1
        }
    ...
    SequentialAnimation {
        ...
        PauseAnimation {
            duration: ShinData.popupAnim(80)
        }
        ...
        NumberAnimation {
            ...
            from: 1.0
            to: 0.0
            duration: ShinData.popupAnim(760)
            easing.type: Easing.OutCubic
        }
    }
    ...
    function clampSelection() {
        ...
        selectedIndex = Math.max(0, Math.min(selectedIndex, historyModel.count))
    }
    ...
    function moveSelection(delta) {
        ...
        if (selectedIndex < 0)
            ...
            selectedIndex = Math.max(0, Math.min(selectedIndex + delta, historyModel.count))

        ensureSelectionVisible()
    }

    function ensureSelectionVisible() {
        if (selectedIndex >= 0 && selectedIndex < historyModel.count)
            historyView.positionViewAtIndex(selectedIndex, ListView.Contain)
    }
    ...
    function activateSelection() {
        ...
        if (selectedIndex === historyModel.count) {
            clearHistory()
            return
        }

        removeHistoryAt(selectedIndex)
    }
    ...
    ListModel {
        id: historyModel
        onCountChanged: win.clampSelection()
    }
    ...
    Connections {
        ...
        function onInsideNonceChanged() {
            if (!win.panelOpen || ShinPopup.active !== "notifications")
                return

            if (ShinPopup.insideY !== 0)
                win.moveSelection(ShinPopup.insideY)
        }

        function onActivateNonceChanged() {
            if (win.panelOpen && ShinPopup.active === "notifications")
                win.activateSelection()
        }
    }
        ...
        stdout: SplitParser {
            onRead: function(lineRaw) {
                ...
                try {
                    ...
                    win.selectedIndex = historyModel.count > 0 ? 0 : -1
                } catch(e) {
        ...
        Rectangle {
            ...
            Item {
                ...
                Row {
                    ...
                    Rectangle {
                        ...
                        height: 30
                        radius: 10
                        color: upArea.containsMouse
                            ? Qt.rgba(ShinColors.accent.r, ShinColors.accent.g, ShinColors.accent.b, 0.16)
                            : Qt.rgba(ShinColors.surface.r, ShinColors.surface.g, ShinColors.surface.b, 0.42)
                        border.width: 1
            ...
            ListView {
                ...
                delegate: Rectangle {
                    ...
                    property string initial: model.app.length > 0 ? model.app.charAt(0).toUpperCase() : "N"
                    property bool selected: win.panelOpen && win.selectedIndex === index
                    ...
                    scale: 0.965 + cardReveal * 0.035 + (selected ? 0.012 : 0.0)
                    ...
                    color: selected
                        ...
                        : Qt.rgba(ShinColors.surface.r, ShinColors.surface.g, ShinColors.surface.b, 0.54)
            ...
            Rectangle {
                ...
                height: 58
                radius: 18
                property bool selected: win.panelOpen && historyModel.count > 0 && win.selectedIndex === historyModel.count
                color: clearAllArea.containsMouse || selected
                    ? Qt.rgba(ShinColors.accent.r, ShinColors.accent.g, ShinColors.accent.b, 0.12)
                    : Qt.rgba(ShinColors.surface.r, ShinColors.surface.g, ShinColors.surface.b, 0.48)
                border.width: selected ? 2 : 1
                border.color: selected
                    ? Qt.rgba(ShinColors.accent.r, ShinColors.accent.g, ShinColors.accent.b, 0.56)
                ...
                MouseArea {
                    ...
                    onEntered: {
                        if (historyModel.count > 0)
                            win.selectedIndex = historyModel.count
                    }
