...
Item {
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: function(data) {
                ...
                var arr = scanProc.tmp.slice()
                arr.push({ kind: kind, path: path, preview: preview, title: title })
                scanProc.tmp = arr
            }
        }
    ...
    PanelWindow {
        ...
        Item {
            ...
            Keys.onPressed: function(event) {
                if (event.key === Qt.Key_Left) {
                    root.moveSelection(root.horizontal ? -1 : 0)
                    event.accepted = true
                } else if (event.key === Qt.Key_Right) {
                    root.moveSelection(root.horizontal ? 1 : 0)
                    event.accepted = true
                } else if (event.key === Qt.Key_Up) {
                    root.moveSelection(root.horizontal ? 0 : -1)
                    event.accepted = true
        ...
        Rectangle {
            ...
            color: Qt.rgba(ShinColors.bg.r, ShinColors.bg.g, ShinColors.bg.b, Math.max(0.58, ShinConfig.popupOpacity))
            border.width: 1
            ...
            Column {
                ...
                ListView {
                    ...
                    spacing: root.horizontal ? 14 : 10
                    model: root.filtered
                    currentIndex: root.selectedIndex
                    highlightMoveDuration: ShinData.anim(root.moveDuration())
                    highlightRangeMode: ListView.ApplyRange
                    preferredHighlightBegin: root.horizontal ? width * 0.34 : height * 0.30
                    preferredHighlightEnd: root.horizontal ? width * 0.66 : height * 0.58
                    cacheBuffer: 420
                    ...
                    delegate: Rectangle {
                        ...
                        radius: ShinConfig.wallpaperModel === 0 ? 999 : 14
                        ...
                        Text {
                            visible: ShinConfig.wallpaperModel !== 0
                            anchors.left: parent.left
                            anchors.right: parent.right
                            anchors.bottom: parent.bottom
                            anchors.leftMargin: 10
