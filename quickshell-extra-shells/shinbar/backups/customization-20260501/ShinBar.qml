...
PanelWindow {
    ...
    Timer {
        ...
        interval: 16
        repeat: true
        running: ShinPopup.focusMode || trailSegments.length > 0
    ...
    Item {
        ...
        Keys.onLeftPressed: win.moveFocus(-1)
        Keys.onRightPressed: win.moveFocus(1)
        Keys.onDownPressed: {
            if (ShinPopup.expandedOpen && ShinPopup.expandedPage === "weather")
                ShinPopup.cycleWeatherFx()
            else
                ShinPopup.openFocused()
        }
        Keys.onReturnPressed: ShinPopup.openFocused()
        Keys.onEnterPressed: ShinPopup.openFocused()
        Keys.onUpPressed: ShinPopup.active = ""
